...
import { ExternalLinkIcon } from 'docs/icons/ExternalLinkIcon';
...
export function Link({ href, className, ...props }: React.ComponentProps<typeof NextLink>) {
  ...
  if (typeof href === 'string' && href.startsWith(process.env.WEBSITE)) {
    href = href.replace(process.env.WEBSITE, '');
  }

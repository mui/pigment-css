  ...
  try {
    ...
    try {
      ...
      if (exp.type !== 'ObjectExpression') {
        throw new Error(`${process.env.PACKAGE_NAME}: The expression must be an object literal.`);
      }
      ...
      throw (
        expression?.buildCodeFrameError(
          `${process.env.PACKAGE_NAME}: Could not parse the expression '${fnString}'.`,
        ) ??
        new Error(`${process.env.PACKAGE_NAME}: Could not parse the given expression ${fnString}`)
      );
    }
  }
}
...
export function valueToLiteral(value: unknown, expression?: ExpressionValue): t.Expression {
  ...
  throw (
    expression?.buildCodeFrameError(
      `${process.env.PACKAGE_NAME}: The expression evaluated to '${value}', which is probably a mistake. If you want it to be inserted into CSS, explicitly cast or transform the value to a string, for example - 'String(${expression.source})'.`,
